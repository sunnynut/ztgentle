...
def main(date):
	...
	hive_sql3="insert overwrite table dwd.dwd_evt_coupon_consume_trace \
			   select a.trace_id,a.order_id,a.team_id,a.operator_id,a.coupon_no,a.operate_type, \
			   a.operate_time,a.operate_count,a.partner_id,a.partner_shop_id,a.coupon_type,coalesce(a.hotel_seq,e.hotel_seq)  \
			   from dwd.dwd_evt_coupon_consume_trace a left outer join(select team_id,min(hotel_seq) as hotel_seq  \
					   from dwd.dwd_com_team_hotel_rel group by team_id) e on a.team_id=e.team_id"
	try:
		print hive_sql1;
