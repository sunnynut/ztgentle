...
case when f.consume_order_count is null then  cast(0 as bigint) else f.consume_order_count end as consume_order_qty,
...
select
team_id, platform_type, generate_order_qty, pay_order_qty, pay_room_night_qty, pay_amount, refund_order_qty,refund_room_night_qty,refund_amount,voucher_room_night_qty, voucher_order_qty 
...
select
team_id,'app',sum(generate_order_qty),sum(pay_order_qty),sum(pay_room_night_qty), sum(pay_amount),sum(refund_order_qty),sum(refund_room_night_qty),sum(refund_amount), sum(voucher_room_night_qty),sum(voucher_order_qty)
from tmp.dwr_team_transaction_di
