...
sudo -u desdev /home/q/hadoop/hive-default/bin/hive <<-EOF
	...
	insert overwrite table dwr.dwr_user_movie_newuser_hotel_di partition (dt='$read_date')
	select a.center_id,a.id,a.user_device_id from
	...
	left outer join
	(select user_center_id,min(create_time) as create_time from tmp.tmp_old_user where dt='$read_date' and substring(create_time,0,10)<='$read_date' and 
	 ((business_type in ('hotel') or (business_type='group' and business_sub_type in ('3001001','3002001')))) group by user_center_id) b
...
EOF
export PGPASSWORD=1adff040-bdbb-4bc0-b9ea-c810af1a7a77&& /usr/bin/psql -h l-tuandb3.des.cn8.qunar.com -U des_report -d qunar_group -p 5432 -c "delete from report.dwr_user_movie_newuser_hotel_di where stat_date='$read_date'"
sudo -u desdev python /home/q/dwetl/lib/py/db/DBLoader/loader.py --from hive:dwr:dwr_user_movie_newuser_hotel_di --to tuandb3:qunar_group:report.dwr_user_movie_newuser_hotel_di --split "\\001" --cond "select dt,user_center_id,first_order_no,user_device_id from dwr.dwr_user_movie_newuser_hotel_di where dt='${read_date}'" --choose choose
