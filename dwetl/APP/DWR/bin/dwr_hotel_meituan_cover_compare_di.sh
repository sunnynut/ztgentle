...
sudo -u desdev /home/q/hadoop/hive-default/bin/hive <<-EOF
	...
	from( select hotel_seq, 
		...
		inner join
		(select hotel_seq,daqu,city_name,hotel_star from dwr.dwr_hotel_info_di where dt='$read_date') q2
		on q1.hotel_seq=q2.hotel_seq
		inner join
		(select quyu,daqu,city_name from dim.dim_city where dt='$read_date') q3
