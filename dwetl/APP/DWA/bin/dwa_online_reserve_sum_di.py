...
def dwa_online_reserve_sum_di():
	...
	(
		select '{date_dt}' as dt,
			   CASE  WHEN o.vid LIKE '9%' THEN 'touch' 
					 WHEN o.vid LIKE '6%' OR o.vid LIKE '8%' THEN 'app' 
					 ELSE 'www' 
					 END AS platform,
			   count(1) as support_reserve_pay_order_qty, 
			   sum(o.quantity * team.baojian_total) as support_reserve_pay_room_night_qty
		from db_sync.order o inner join
		db_sync.team team on o.team_id=team.id
		inner join
		db_sync.team_ext ext on o.team_id=ext.team_id
		where ext.online_booking = 1 and date_format(o.pay_time, '%Y-%m-%d') = '{date_dt}'
		group by CASE  WHEN o.vid LIKE '9%' THEN 'touch' 
					   WHEN o.vid LIKE '6%' OR o.vid LIKE '8%' THEN 'app' 
					   ELSE 'www' 
					   END
	...
	(
		select '{date_dt}' as dt,
			   ...
			   sum(reserve.quantity) as reserve_room_night_qty
		from db_sync.reserve reserve
		inner join db_sync.order orde 
		on reserve.oid=orde.id and date_format(reserve.create_time, '%Y-%m-%d') = '{date_dt}'
		inner join db_sync.team team on reserve.team_id=team.id
		where reserve.request_source!=4
		group by CASE  WHEN orde.vid LIKE '9%' THEN 'touch' 
					   WHEN orde.vid LIKE '6%' OR orde.vid LIKE '8%' THEN 'app' 
					   ELSE 'www' 
					   END
	...
	(
		select '{date_dt}' as dt,
			   ...
			   sum(reserve.quantity) as reserve_success_room_night_qty
		from db_sync.reserve reserve
		inner join db_sync.order orde 
		on reserve.oid=orde.id and date_format(reserve.create_time, '%Y-%m-%d') = '{date_dt}'
		inner join db_sync.team team on reserve.team_id=team.id
		where reserve.status in (1,2) and reserve.request_source!=4
		group by CASE  WHEN orde.vid LIKE '9%' THEN 'touch' 
					   WHEN orde.vid LIKE '6%' OR orde.vid LIKE '8%' THEN 'app' 
					   ELSE 'www' 
					   END
	...
	(
		select '{date_dt}' as dt,
			   ...
			   sum(reserve.quantity) as reserve_fail_room_night_qty
		from db_sync.reserve reserve
		inner join db_sync.order orde 
		on reserve.oid=orde.id and date_format(reserve.create_time, '%Y-%m-%d') = '{date_dt}'
		inner join db_sync.team team on reserve.team_id=team.id
		where reserve.status = -1 and reserve.request_source!=4
		group by CASE  WHEN orde.vid LIKE '9%' THEN 'touch' 
					   WHEN orde.vid LIKE '6%' OR orde.vid LIKE '8%' THEN 'app' 
					   ELSE 'www' 
					   END
	...
	(
		...
		inner join db_sync.order orde 
		on reserve.oid=orde.id and date_format(reserve.create_time, '%Y-%m-%d') = '{date_dt}'
		where reserve.status in (-2,-4,-5) and reserve.request_source!=4
		group by CASE  WHEN orde.vid LIKE '9%' THEN 'touch' 
					   WHEN orde.vid LIKE '6%' OR orde.vid LIKE '8%' THEN 'app' 
					   ELSE 'www' 
					   END
	...
	(
		...
		inner join db_sync.order orde 
		on reserve.oid=orde.id and date_format(reserve.create_time, '%Y-%m-%d') = '{date_dt}'
		where reserve.status in (-3) and reserve.request_source!=4
		group by CASE  WHEN orde.vid LIKE '9%' THEN 'touch' 
					   WHEN orde.vid LIKE '6%' OR orde.vid LIKE '8%' THEN 'app' 
					   ELSE 'www' 
					   END
