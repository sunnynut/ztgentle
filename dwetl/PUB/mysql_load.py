...
for row in result:
    ...
    for i in range(startwith,3) :
        filename=table_name+ str(i)+'.txt'
        file_path="/ROOT/LOGS/TB_DATA/%s/%s" % (date,filename)
        if is_increment==0:
            sql=sql_select+sql_from + str(i)
        else :
            sql_filter=sql_filter.replace('$data_dt_next','\'%s\'' % day).replace('$data_dt','\'%s\'' % last_day)
            sql=sql_select+sql_from+ str(i)+sql_filter
        for row2 in result2 :
            db_ip,db_port,db_user,db_password,db_name=row2
            mysql="mysql -h %s -P %s -u %s -p\"%s\" -N -D %s -e \"%s\" >%s" % (db_ip,db_port,db_user,db_password,db_name,sql,file_path)
            if os.system(mysql)!=0 :
                sys.exit(1)
            if os.system('gzip '+file_path)!=0 :
                sys.exit(1)
            hql="hive -e \"load data local inpath '%s.gz' overwrite into table stg.%s partition(dt='%s',dp='%s');\"" % (file_path,hive_tb_name,last_day,i)
            if os.system(hql)!=0 :
                sys.exit(1)
