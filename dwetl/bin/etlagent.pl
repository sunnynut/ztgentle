...
sub createLogFile
{
   ...
   $LOGFILE = "${AUTO_LOG}/etlagent_${TODAY}.log";
...
sub getConfigure
{
   my $cfgfile = "${AUTO_HOME}${DIRDELI}etc${DIRDELI}agent.cfg";
   my @vars;
   ...
   unless(open(CFGFILE_H, "$cfgfile")) {
      return;
   }
    
   my $line;
   while($line=<CFGFILE_H>) {
      $line = cutLeadingTrailSpace($line);
      if ("$line" ne "" || substr($line, 0, 1) ne "#") {
         @vars = split(/\s+/, $line);
         if ("$vars[0]" eq "INVOKE") {
            $INVOKEFLAG = $vars[1];
            if (!defined($INVOKEFLAG)) { $INVOKEFLAG = 0; }
         } elsif ("$vars[0]" eq "FORCE") {
            $FORCEFLAG = $vars[1];
            if (!defined($FORCEFLAG)) { $FORCEFLAG = 0; }  	
         } elsif ("$vars[0]" eq "GETLOG") {
            $GETLOGFLAG = $vars[1];
            if (!defined($GETLOGFLAG)) { $GETLOGFLAG = 0; }  	
         } elsif ("$vars[0]" eq "GETSCR") {
            $GETSCRFLAG = $vars[1];
            if (!defined($GETSCRFLAG)) { $GETSCRFLAG = 0; }  	
         } elsif ("$vars[0]" eq "PUTSCR") {
            $PUTSCRFLAG = $vars[1];
            if (!defined($PUTSCRFLAG)) { $PUTSCRFLAG = 0; }  	
         }
      }
   }
...
{
   my $acceptList = "${AUTO_HOME}/etc/agent.host";
...
sub serviceRoutine
{
   ...
   print $Connection ("TASK Agent, Version ${VERSION}${NEWLINE}");
   print $Connection ("Copyright 2013 DJ${NEWLINE}");
...
sub cmdINVOKE
{
   ...
   if ($INVOKEFLAG != 1) {  # Not support this function
      showTime(); print "Not support INVOKE function\n";
      if ( $DEBUG_FLAG == 1 ) {
         print STDOUT "Not support INVOKE function\n";
      }
...
sub cmdFORCE
{
   ...
   if ($FORCEFLAG != 1) {  # Not support this function
      showTime(); print "Not support FORCE function\n";
      if ( $DEBUG_FLAG == 1 ) {
         print STDOUT "Not support FORCE function\n";
      }
...
sub cmdGETLOG
{
   ...
   if ($GETLOGFLAG != 1) {  # Not support this function
      showTime(); print "Not support GETLOG function\n";
      if ( $DEBUG_FLAG == 1 ) {
         print STDOUT "Not support GETLOG function\n";
      }

      print $con ("${ERROR} GETLOG${NEWLINE}");
      return;   	
   }
...
sub cmdGETSCR
{
   ...
   if ($GETSCRFLAG != 1) {  # Not support this function
      showTime(); print "Not support GETSCR function\n";
      if ( $DEBUG_FLAG == 1 ) {
         print STDOUT "Not support GETSCR function\n";
      }

      print $con ("${ERROR} GETSCR${NEWLINE}");
      return;   	
   }
...
sub cmdPUTSCR
{
   ...
   if ($PUTSCRFLAG != 1) {  # Not support this function
      showTime(); print "Not support PUTSCR function\n";
      if ( $DEBUG_FLAG == 1 ) {
         print STDOUT "Not support PUTSCR function\n";
      }

      print $con ("${ERROR} PUTSCR${NEWLINE}");
      return;   	
   }
...
sub printVersionInfo
{
   print "\n";
   ETL::showTime(); print "***********************************************************\n";
   ETL::showTime(); print "* WorkFlow Agent Program ${VERSION}, DJ 2013 Copyright. *\n";
   ETL::showTime(); print "***********************************************************\n";
   print "\n";
}
