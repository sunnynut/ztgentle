...
my $ETLDIR;
my $ETL_DSN;
my $ETL_HOST; #add by dj for mysql
my $ETL_SERVER;
...
#add by dj for mysql 
$ETL::ETL_HOST = $ENV{"AUTO_HOST"};
if ( !defined($ETL::ETL_HOST) ) {
   $ETL::ETL_HOST = "192.168.233.16";
}

$ETL::ETL_ETC  = "${ETL::ETLDIR}/etc";
$ETL::ETL_BIN  = "${ETL::ETLDIR}/bin";
$ETL::ETL_TMP  = "${ETL::ETLDIR}/tmp";
...
$ETL::PERLPATH = "/usr/bin/perl";

$ETL::PERL = "perl";

$ETL::INVOKER = "perl ${ETL::ETL_BIN}/etlslave_unix.pl";
...
sub getComment
{
	my ($dbh,$Database,$Tablename) = @_;
    #my $sqlText = "SELECT commentstring FROM dbc.tables WHERE databasename='${Database}' AND tablename='${Tablename}'";
    #modify by dj 2013/06/28 for mysql
    my $sqlText = "select t.TABLE_COMMENT from information_schema.TABLES t where t.TABLE_SCHEMA='${Database}' and TABLE_NAME='${Tablename}'";
    my $sth = $dbh->prepare($sqlText);
    unless($sth) {
...
sub getJobRunInfo
{
   ...
   unless ($sth) {
      ...
      $jobType = "";
      $txdate = "";
      return ($etlsys, $etljob, $autoOff, $txdate, $jobType);
   }
...
sub isGroupChildOK1
{
   ...
   my $sqltext = "SELECT count(*) FROM ${ETL::ETLDB}ETL_Job_GroupChild" .
                 ...
                 "     AND Enable = '1'" .
                 "     AND (CheckFlag <> 'Y' OR CheckFlag is null)";
...
sub isTriggerByTime
{
   ...
   my $sqlText = "SELECT TimeTrigger FROM ${ETL::ETLDB}ETL_Job" .
                 "   WHERE ETL_System = '$sys'" .
                 "     AND ETL_Job = '$job'";
...
sub createDirectory
{
   ...
   unless (mkdir($dir, 0770)) { return $ETL::FALSE; }
   return $ETL::TRUE;
}
...
sub invokeJob
{
   ...
   $job = "nohup ${ETL::INVOKER} $param >/dev/null 2>&1 &";

   showPrefixSpace(); print "$job\n";
...
{  
   my ($logonstring)=@_	  ;
   my ($ETLLOGON) = "${ETL::ETL_ETC}/ETL_LOGON";     
   if (defined($logonstring)) {   	
   	$ETLLOGON= "${ETL::ETL_ETC}/$logonstring" ;      	
   }
   open(LOGONF, $ETLLOGON);
   my $logon =<LOGONF>;
   close(LOGONF);
 ...
 #  my $dbh = DBI->connect("dbi:Teradata:${ETL::ETL_DSN}", $user, $decodepass,
 #                          { AutoCommit => 1, PrintError => 0, RaiseError => 0 } ) ;
 #切换到odbc连接 by dj 
  #my $dbh = DBI->connect("dbi:ODBC:${ETL::ETL_DSN}", $user, $decodepass,
  #                       { AutoCommit => 1, PrintError => 0, RaiseError => 0 } ) ;
  #切换到dbd-mysql连接mysql，需要安装mysql-connect,DBI-1.627,DBD-mysql-4.023
  my $dbh = DBI->connect("dbi:mysql:${ETL::ETL_DSN}:${ETL::ETL_HOST}", $user, $passwd,
                          { AutoCommit => 1, PrintError => 0, RaiseError => 0 } ) ;
