...
sub exportFastloadE1
{
   ...
   $cmd = "perl ${ETL::ETL_BIN}/ExportFastLoadER1.pl \"${er1_table}\" \"${outfile}\"";
...
sub exportMultiloadET
{
   ...
   $cmd = "perl ${ETL::ETL_BIN}/ExportMultiloadET.pl \"${et_table}\" \"${outfile}\"";
...
sub getJobFailInfo
{
   ...
   my $sqlText = "SELECT Etl_Job,Last_TXDate,main_man FROM ${ETL::ETLDB}ETL_Job" .
                " WHERE ETL_Job = '$job'";
...
sub mainRealJob
{
   ...
   unless ( -d $AppPath ) {
      ETL::showPrefixSpace(); print ("Warnning: $AppPath does not exist!\n");
#      return $FALSE;
   }

   unless ( -d $AppBINPath ) {
      ETL::showPrefixSpace(); print ("Warnning: bin directory does not exist!\n");
#      return $FALSE;
   }
  ...
  $PythonScript ="${AppPythonBinPath}/${jobscript}.sh";}
   ETL::showPrefixSpace(); print ("PythonScript=${PythonScript}\n");
      ...
      my $eventDesc = "[$Sys], [$Job] no any job script exiting";
      ETL::insertEventLog($dbCon, "SLV", "H", "$eventDesc");

      my $cur_time = ETL::getCurrentDateTime();
...
#              substr($filename, length($filename) - 3, 3) eq ".py") {      
...
#   my @sortList = sort(@dirFileList);
   ...
   $cur_time = ETL::getCurrentDateTime();
   ETL::updateJobEndTime($dbCon2, $Sys, $Job, $cur_time);
   ETL::updateJobStatusWithCheckFlag($dbCon2, $Sys, $Job, $status, $checkflag);

   # Set the group child job check flag for 'Y' if job was done
   # Otherwise, set it to 'N'
   updateGroupChildCheckFlag($dbCon2, $Sys, $Job, $checkflag, $txdate);

   # Update the job trace record 
   updateJobTraceEnd($dbCon2, $Sys, $Job, $txdate, $status, $cur_time);

   ETL::showPrefixSpace(); print "Log Job status\n";
   ETL::clearJobStatusLog($dbCon2, $Sys, $Job, $txdate, $JobSessionID);
   ETL::insertJobStatusLog($dbCon2, $Sys, $Job, $txdate, $JobSessionID);
   ETL::showTime(); print "Disconnect from ETL DB2...\n";
   ...
   if ( "$status" eq "Done" && $RunDownstreamFlag == 1 ) {
      ...
      ETL::showTime(); print "Invoke [$#jobstream] downstream job\n";
...
sub printVersionInfo
{
   print "\n";
   ETL::showTime(); print "*********************************************************************\n";
   ETL::showTime(); print "* WorkFlow Slave Program for Unix ${VERSION}, DJ 2013 Copyright. *\n";
   ETL::showTime(); print "*********************************************************************\n";
   print "\n";
}
...
sub getJobRunInfo
{
   ...
   my ($etljob, $autoOff, $jobType,$retrycount);
   my $sqltext;
   ...
   unless ($sth) {
      ...
      $txdate = "";
      $retrycount = 0;
      return ($etlsys, $etljob, $autoOff, $txdate, $jobType, $retrycount);
   }
   ...
   $sqltext = "SELECT AutoOff, JobType, Retry_Count FROM ${ETL::ETLDB}ETL_Job" .
...
if ( $#ARGV == 0 ) {
   ...
   ($Sys, $Job, $AutoOff, $TxDate, $JobType, $RetryCount) = getJobRunInfo($dbCon, $ControlFile);
   if ( "$Sys" eq "" || "$Job" eq "" ) {
   ...
   ($AutoOff, $JobType, $RetryCount) = ETL::getJobRunInfo1($dbCon, $Sys, $Job);
   print STDOUT "AutoOff:[$AutoOff] JobType:[$JobType] RetryCount:[$RetryCount]\n";
   if ( "$AutoOff" eq "" ) {
...
ETL::showTime(); print "System:[$Sys] Job:[$Job] AutoOff:[$AutoOff] TXDate:[$TxDate] RetryCount:[$RetryCount]\n";
