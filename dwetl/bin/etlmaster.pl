...
sub moveToProcess
{
   ...
   for ($n = 0; $n <= $#list ; $n++) {
      ...
      # If the file name is empty, we skip it
      # otherwise, it will move whole queue directory to process
      if ("$file" eq "") {
      	 next;
      }
...
sub moveBackToQueue
{
   ...
   for ($n = 0; $n <= $#list ; $n++) {
      ...
      # If the file name is empty, we skip it
      # otherwise, it will move whole queue directory to queue
      if ("$file" eq "") {
      	 next;
      }
...
sub isDependantJobRunning
{
   ...
   $sqltext = "SELECT ETL_System, ETL_Job" .
              "   FROM ${ETL::ETLDB}ETL_Job_Dependency" .
              "   WHERE Dependency_System = '$sys'" .
              "     AND Dependency_Job = '$job'" ;
   ...
   for (my $i=0; $i < $n; $i = $i + 2) { 
      ...
      @status = $sth1->fetchrow();
      $sth1->finish();

      if ("$status[0]" eq "Running") {
         $isrunning = 1;
         last;
      }
   }
...
sub isDependantByJobRunning
{
   ...
   $sqltext = "SELECT Dependency_System, Dependency_Job" .
              "   FROM ${ETL::ETLDB}ETL_Job_Dependency" .
...
sub isHeadJobRunning
{
   ...
   $sqltext = "SELECT B.ETL_System, B.ETL_Job" .
              "  FROM ${ETL::ETLDB}ETL_Job_GroupChild A, ${ETL::ETLDB}ETL_Job_Group B" .
              " WHERE A.ETL_System = '$sys' AND A.ETL_Job = '$job'" .
...
sub isChildJobRunning
{
   ...
   $sqltext = "SELECT B.ETL_System, B.ETL_Job" .
              "  FROM ${ETL::ETLDB}ETL_Job_Group A, ${ETL::ETLDB}ETL_Job_GroupChild B" .
              " WHERE A.ETL_System = '$sys' AND A.ETL_Job = '$job'" .
              "   AND A.GroupName = B.GroupName";
   ...
   my ($childsys, $childjob);
   my $isrunning = 0;
   my @status;
   
   for (my $i=0; $i < $n; $i = $i + 2) { 
      $childsys = $childtab[$i];
      $childjob = $childtab[$i + 1];

      $sqltext1 = "SELECT Last_JobStatus FROM ${ETL::ETLDB}ETL_Job" .
                  "   WHERE ETL_System = '$childsys'" .
                  "     AND ETL_Job = '$childjob'";
...
#   }
   ...
   # Check to see if any of dependant job is running.
   # If so, we do not allow the job to run
   ETL::showPrefixSpace(); print "Check Dependant Job Running\n";
   if (isDependantJobRunning($dbCon, $sys, $job) ||
       isDependantByJobRunning($dbCon, $sys, $job)) {
      ETL::showPrefixSpace(); print "There is dependant job running, wait for next time!\n";

      if ( isJobAlreadyHasEvent($sys, $job, "Dependant Job Running") ) {
         return $FALSE;
      }
   ...
   ETL::showPrefixSpace(); print "Check Group Head Job Running\n";
   if (isHeadJobRunning($dbCon, $sys, $job)) {
      ETL::showPrefixSpace(); print "There is group head job running, wait for next time!\n";

      if ( isJobAlreadyHasEvent($sys, $job, "Head Job Running") ) {
         return $FALSE;
      }
   ...
   ETL::showPrefixSpace(); print "Check Group Child Job Running\n";   
   if (isChildJobRunning($dbCon, $sys, $job)) {
      ETL::showPrefixSpace(); print "There is group child job running, wait for next time!\n";

      if ( isJobAlreadyHasEvent($sys, $job, "Child Job Running") ) {
         return $FALSE;
      }
...
#      ETL::showTime(); print "$errstr\n";
   ...
   $sqlText = "SELECT SeqID, ETL_System, ETL_Job, TXDate " .
              "   FROM ${ETL::ETLDB}ETL_Job_Queue" .
              "   WHERE ETL_Server = '$server' AND " .
              "      (ETL_System, ETL_JOB, SeqID) IN " .
	          "   (select ETL_System, ETL_JOB, SeqID FROM ${ETL::ETLDB}ETL_Job_Queue " .
...
sub printVersionInfo
{
   print "\n";
   ETL::showTime(); print "************************************************************\n";
   ETL::showTime(); print "* WorkFlow Master Program {$VERSION}, DJ 2013 Copyright. *\n";
   ETL::showTime(); print "************************************************************\n";
   print "\n";
