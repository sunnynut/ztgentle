...
sub exportFastloadE1
{
   ...
   if ( defined(${ETL::PERLPATH}) && "${ETL::PERLPATH}" ne "" ) {
      $cmd = "${ETL::PERLPATH}\\bin\\perl.exe ${ETL::ETL_BIN}\\ExportFastLoadER1.pl \"${er1_table}\" \"${outfile}\"";
   } else {
      $cmd = "perl.exe ${ETL::ETL_BIN}\\ExportFastLoadER1.pl \"${er1_table}\" \"${outfile}\"";
   }
...
sub exportMultiloadET
{
   ...
   if ( defined(${ETL::PERLPATH}) && "${ETL::PERLPATH}" ne "" ) {
      $cmd = "${ETL::PERLPATH}\\bin\\perl.exe ${ETL::ETL_BIN}\\ExportMultiLoadET.pl \"${et_table}\" \"${outfile}\"";
   } else {
      $cmd = "perl.exe ${ETL::ETL_BIN}\\ExportMultiLoadET.pl \"${et_table}\" \"${outfile}\"";
   }
...
sub updateRecordLog
{
   ...
   # If the fastload error table 1 has record,
...
sub invokeJob
{
   ...
   if ( defined(${ETL::PERLPATH}) && "${ETL::PERLPATH}" ne "" ) {
      $job = "${ETL::PERLPATH}\\bin\\perl.exe ${AppBINPath}\\${scriptFile} ${controlFile} > ${logfile}";
   } else {
      $job = "perl.exe ${AppBINPath}\\${scriptFile} ${controlFile} > ${logfile}";
   }
   ...
   $ret = $ret >> 8;
	unless ( $dbCon->ping() ) {
		ETL::showTime(); print "Warning - Lost database connection. Reconnect to ETL repository.\n";
...
sub invokeJob1
{
   ...
   if ( defined(${ETL::PERLPATH}) && "${ETL::PERLPATH}" ne "" ) {
      $perlpath = "${ETL::PERLPATH}\\bin\\perl.exe";
   } else {
      $perlpath = "perl.exe";
   }
   
   $job = "perl.exe ${AppBINPath}\\${scriptFile} $controlFile";
   ...
   if ( Win32::Process::Create($processObj, "$perlpath", $job, 1,
                               NORMAL_PRIORITY_CLASS|CREATE_NO_WINDOW                 
                               , "${home}") )
   {
       my $pid = $processObj->GetProcessID();
       while (1) {
          $ret = $processObj->GetExitCode($exitcode);
      	 
          # If the process is still running, we wait for 3 seconds and check it again
...
sub mainRealJob
{
   ...
   while ($filename = readdir(BIN_DIR))
   {
      ...
      # To check if it is a perl script file
      if ( substr($filename, 0, length($Job)) eq $jobscript ) {
         if ( substr($filename, length($filename) - 3, 3) eq ".pl" ) {      
   ...
   # If there no perl script file existing, we write a log message into log file
   # then return from this funtion
   if ($n == 0) {
      ETL::showTime(); print ("No perl script file existing!\n");
   ...
   # Processing the job script file file one by one.
   for (my $i=0; $i < $n; $i++)
   {
      if ( $STOP_FLAG ) { last; }
      ...
      $ret = invokeJob($filename, $ControlFile);
      #$ret = invokeJob1($filename, $ControlFile);
...
# Modified by bill   2003年4月2日
#   my ($dbh, $server, $sys, $job, $txdate) = @_;
#               if (putJobIntoQueue($dbCon, $server, $TxDate, $gsys, $gjob)==$FALSE) {
               if (putJobIntoQueue($dbCon, $server, $gsys, $gjob, $TxDate)==$FALSE) {
                  ETL::showPrefixSpace(); print "ERROR - Put into job queue failed!\n";
               }
            }
         }
...
sub printVersionInfo
{
   print "\n";
   ETL::showTime(); print "*******************************************************************\n";
   ETL::showTime(); print "* WorkFlow Slave Program for NT ${VERSION}, DJ 2013 Copyright. *\n";
   ETL::showTime(); print "*******************************************************************\n";
   print "\n";
}
...
sub printSyntax
{
   print STDERR "etlslave_nt.pl: Automation Job Script Runner, v2.5.1\n";
   print STDERR "Syntax 1: etlslave_nt.pl <control file>\n";
   print STDERR "          <control file> the control file name for executing job.\n";
   print STDERR "Syntax 2: etlslave_nt.pl <sys> <job> <txdate> [-nd] [-ng]\n";
