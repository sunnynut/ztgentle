...
sub checkDataFileSize
{
   ...
   for (my $n=0; $n <= $count; $n++)
   {
      ...
      $file = $fields[0];
# Add by Bill 20030810
# If the size is greater than 2GB, we do not check filesize
	  if ($fields[1] > 2048000000) {
		return $ret;
	  }
   ...
   for (my $n=0; $n <= $count; $n++)
   {
      ...
      if ( ETL::checkSourceFileDuplicate($dbCon, $sys, $job, $file) ) {
         ...
         ETL::insertEventLog($dbCon, "RCV", "M", "$eventDesc");
      	 ...
      	 # Save data file name to dataFileList1 array
      	 $dataFileList1[$p] = $dataFileList[$n];
      	 $p++;
      }
   }
...
sub checkDataFileDuplicate1
{
   ...
   for (my $n=0; $n <= $count; $n++)
   {
      ...
      if ( ETL::checkSourceFileDuplicate($dbCon, $sys, $job, $file) ) {
         ETL::showPrefixSpace(); print "The source file '$file' is already received but we filter it out!\n";
         $ret = $FALSE;
         
         $eventDesc = "[$sys], [$job] has received duplicate file $file";
         ETL::insertEventLog($dbCon, "RCV", "M", "$eventDesc");
         $tempList[$p1] = $dataFileList[$n];
         $p1++;
      } else {
      	 # Save data file name to dataFileList1 array
      	 $dataFileList1[$p2] = $dataFileList[$n];
      	 $p2++;
      }
   }
   
   # Because there is some data file duplicate
   # We reset the dataFileList array and set only dupilicate file name
   if ($ret == $FALSE) {
      @dataFileList = ();
...
sub moveDataFileToQueue
{
   ...
   my $count = $#dataFileList1;

   for (my $n=0; $n <= $count; $n++)
   {
      @fields = split(/\s+/, $dataFileList1[$n]);
...
sub convertControlFile
{
   ...
   ETL::showTime(); print "newControlFile =$newControlFile\n";
   ETL::showTime(); print "${ETL::ETL_TMP}${DIRDELI}${newControlFile}";
   ...
   for (my $n = 0; $n <= $count ; $n++) {
      ...
      print NEWCTRLFILEH ("$dataFileList1[$n]\n");
      ...
      ETL::showPrefixSpace(); print "Insert source '$file' log.\n";
      ...
      unless ( $ret ) {
         ...
         my $errstr = $dbCon->errstr;
         ETL::showPrefixSpace(); print "$errstr\n";
      }
   }
...
sub convertControlFile1
{
   ...
   unless ( open(NEWCTRLFILEH, ">${ETL::ETL_TMP}${DIRDELI}$newControlFile") ) {
      ETL::showPrefixSpace(); print "ERROR - Can not open converted control file1.\n";
      return "";
   }
...
{
   my ($dbh, $sys, $job, $checkmode, $txdate) = @_;
   my ($retYear, $retMonth, $retDay);
   my @tabrow;
   my $sqlText;
  
   ETL::showPrefixSpace(); print "Get prior calendar date, $sys, $job, $checkmode, $txdate\n";
   if ( $checkmode eq "0") {
      $sqlText = 
			" SELECT MAX(concat(lpad(CalendarYear,4,0),lpad(CalendarMonth,2,0),lpad(CalendarDay,2,0)))          \n".
			" FROM ${ETL::ETLDB}DataCalendar                                        \n".
			" WHERE ETL_System = '$sys' AND ETL_Job = '$job'                        \n".
			"      AND concat(lpad(CalendarYear,4,0),lpad(CalendarMonth,2,0),lpad(CalendarDay,2,0)) <='$txdate' ";
	} else {
      $sqlText = 
			" SELECT MAX(concat(lpad(CalendarYear,4,0),lpad(CalendarMonth,2,0),lpad(CalendarDay,2,0)))   \n".
		  " FROM ${ETL::ETLDB}DataCalendar                                          \n".
   ...
   if (@tabrow = $sth->fetchrow()) {
	  $retYear  = substr($tabrow[0],0,4) + 0;
      $retMonth = substr($tabrow[0],4,2) + 0;
      $retDay   = substr($tabrow[0],6,2) + 0;
   }
...
sub checkJobPriorStatus
{
   ...
   my ($pyear, $pmonth, $pday) = getPriorCalendarDate($dbh, $sys, $job, $checkmode, $txdate);
...
{
   my ($sys, $job, $txdate, $type, $subject, $content) = @_;
   my $msgControlFile;
   my $current;
...
sub compareCurrentTxDate
{
   ...
   $sqltext = "SELECT Last_TXDate,DATE_ADD(DATE_FORMAT(Last_TXDate,'%Y-%m-%d'), INTERVAL 1 DAY)" . 
   ...
   ETL::showTime(); print "The current job txdate is '$tabrow[0]'\n";

   my $curtxdate = substr($tabrow[0], 0, 10);
   my $tomtxdate = substr($tabrow[1], 0, 10);
   ETL::showTime(); print "Compare current txdate '$curtxdate' with new txdate '$txdate'\n";
   
   if ( "$tomtxdate" eq "$txdate" ) {
      return $TRUE;
   }
   else{
   return $FALSE;
...
sub getSourceAndTxDate
{
   ...
   if ( $NameMode == $SHORT_NAME_MODE ) {
      $source = substr($file, 1, 5);
      $txdate = substr($file, 6, 4);
      ...
      $tempfile = substr($file, 4);
      my $loc = rindex($tempfile, ".");

      if ($loc != -1) {
      	 $tempfile = substr($tempfile, 0, $loc);
      }
	 
      $source = substr($tempfile, 0, (length($tempfile) - 8));
      $txdate = substr($tempfile, (length($tempfile) - 8), 8);
   }
   elsif ( $NameMode == $EVA_NAME_MODE ) {
      $tempfile = substr($file, 4);
      my $loc = rindex($tempfile, ".");
      my $len = length($tempfile);
      
      if ($loc != -1 && ($len - $loc) <= 8) {
      	 $tempfile = substr($tempfile, 0, $loc);      	
      }

      $source = substr($tempfile, 0, (length($tempfile) - 9));
      $txdate = substr($tempfile, (length($tempfile) - 8), 8);
   }

   return ($source, $txdate);
}
...
sub processControlFile
{
   ...
   ($jobSource, $txDate) = getSourceAndTxDate($controlFile);
   ...
   # We check the system and job are same as the last pending or running job
   # If they are same then we just skip it
   if ( "$LastSysName" eq "$sysName" && "$LastJobName" eq "$jobName" ) {
      ETL::showPrefixSpace(); print "WARNING - Same job has been checked, we skip it.\n";      
      return $FALSE;
   }
   ...
   if ( ETL::isJobCheckLastStatus($dbCon, $sysName, $jobName) == $TRUE ) {
      # Get the job current status
      $currentStatus = ETL::getJobStatus($dbCon, $sysName, $jobName);
      if ( !("$currentStatus" eq "" || "$currentStatus" eq "Ready" ||
             "$currentStatus" eq "Done" )) {
         # The current job status is not in 'Done' or 'Ready',
      ...
      # Get the job current status
      $currentStatus = ETL::getJobStatus($dbCon, $sysName, $jobName);
      if ( "$currentStatus" eq "Pending" || "$currentStatus" eq "Running" ) {
         # The current job status is still in 'Pending' or 'Running',
         # we don't allow to receiving file at the moment
         ETL::showPrefixSpace(); print "WARNING - The job is in $currentStatus, we will process this file next time.\n";
   ...
   unless ( checkDataFileSize($controlFile) ) {
      ...
      $eventDesc = "[$sysName], [$jobName] has corrupt data in control file $controlFile";
      ETL::insertEventLog($dbCon, "RCV", "H", "$eventDesc");

      removeJobEventRecord($sysName, $jobName);

      ETL::showTime(); print "Generate data file corrupt message...\n";
      writeMessageNotification($sysName, $jobName, $txDate,
                               "Receiving",
                               "Job [$sysName,$jobName] has corrupt data in control file $controlFile",
                               "");
   ...
   unless ( checkJobFrequency($sysName, $jobName, $txDate) ) {
      ...
      ETL::showTime(); print "Generate data file frequency did not match message...\n";
      writeMessageNotification($sysName, $jobName, $txDate,
                               "Receiving",
                               "Job [$sysName,$jobName] has received a control file $controlFile did not match the frequency",
                               "");
   ...
   unless ( checkDataCalendar($dbCon, $sysName, $jobName, $txDate) ) {
      ...
      ETL::showTime(); print "Generate data file did not match data calendar message...\n";
      writeMessageNotification($sysName, $jobName, $txDate,
                               "Receiving",
                               "Job [$sysName,$jobName] has received a control file $controlFile did not match the data calendar",
                               "");
   ...
   if ( compareCurrentTxDate($dbCon, $sysName, $jobName, $txDate) == $FALSE ) {
      moveDataFileToByPass($controlFile, $sysName);
      moveControlFileToByPass($controlFile, $sysName);

      ETL::showTime(); print ("ERROR - The TxDate '$txDate' is not eq the job current TxDate.\n");

      $eventDesc = "[$sysName], [$jobName] has received a control file, whose txdate is not eq the job current txdate";
      ETL::insertEventLog($dbCon, "RCV", "M", "$eventDesc");

      removeJobEventRecord($sysName, $jobName);

      ETL::showTime(); print "Generate data txdate is not eq the job current txdate message...\n";
      writeMessageNotification($sysName, $jobName, $txDate,
                               "Receiving",
                               "Job [$sysName,$jobName] has received a control file, whose txdate is not eq the job current txdate",
                               "");
      
      return $FALSE;	
   }
   
   my $autoFilter = ETL::getJobSourceInfo($dbCon, $jobSource);
   
   # If this source allow auto filter out duplicate data file
   if ( "$autoFilter" eq "1" ) {
      unless ( checkDataFileDuplicate1($sysName, $jobName) ) {
         moveDataFileToDuplicate($sysName);
      ...
      unless ( checkDataFileDuplicate($sysName, $jobName) ) {
         ...
         moveControlFileToDuplicate($controlFile, $sysName);
         removeJobEventRecord($sysName, $jobName); 
         ETL::showTime(); print "Generate data file duplicate message...\n";
         writeMessageNotification($sysName, $jobName, $txDate,
                                  "Receiving",
                                  "Job [$sysName,$jobName] has received duplicate file",
                                  "");
...
# We check to see if the same job's control file name is already in
# the control file list arry. If do, we return TRUE, otherwise, return FALSE
sub isAlreadyExist
{
   my ($filename, $p) = @_;
   my $i;
   ...
   if ( $NameMode == $SHORT_NAME_MODE ) {
      $fname = substr($filename, 0, 6);
   }
   elsif ( $NameMode == $LONG_NAME_MODE ) {
      $fname = substr($filename, 0, (length($filename) - 8));
   }
   elsif ( $NameMode == $EVA_NAME_MODE ) {
      $fname = substr($filename, 0, (length($filename) - 9));
   }
   
#   $flen = length($fname);     Modify By Bill 2004-06-17
   
   for ($i=0; $i<$p; $i++) {
#      $temp = substr($dirFileList[$i], 0, $flen);   Modify By Bill 2004-06-17  This line is replaced by next line
      $temp = substr($dirFileList[$i], 0, length($dirFileList[$i])-8);
      if ( "$fname" eq "$temp" ) {
      	 $result = $TRUE;
         last;
      }
...
sub checkReceiveDir
{
   ...
   unless ( opendir(RCV_DIR, "${ETL::ETL_RECEIVE}") ) {
      ETL::showTime(); print "ERROR - Unable to open '${ETL::ETL_RECEIVE}'\n";
      return $FALSE;
   }
   ...
   ETL::showTime(); print "Connect to WorkFlow repository...\n";
   $dbCon = ETL::connectETL();

   unless ( defined($dbCon) ) {
      ETL::showTime(); print "ERROR - Unable to connect to WorkFlow repository!\n";
...
#      ETL::showTime(); print "$errstr\n";
   ...
   for (my $i=0; $i < $n; $i++)
   {
      ...
      if ( isAlreadyExist($filename, $i) == $TRUE ) {
      	 ETL::showTime(); print "Warnning - Same job's control file '$filename' is already in list before, we skip it this time!\n";
      	 next;
      }
   ...
   ETL::showTime(); print "Disconnect from WorkFlow repository...\n";
...
#      ETL::showTime(); print "$errstr\n";
   ...
   for ($i=0; $i <$n; $i = $i+5) {
       ...
       writeMessageNotification($sys, $job, $checkingday,
                                "Missing",
                                "Job [$sys, $job] did not receive job source file for ${checkingday}",
                                "");
...
#      $MutexObj = ETL::CreateMutex("ETLMSG_MUTEX");
	  my @words = split(/\\/, $home);
	  my $APP= $words[$#words];
      $MutexObj = ETL::CreateMutex("${APP}RCV_MUTEX");
      if ( ! defined($MutexObj) ) {
...
sub printVersionInfo
{
   print "\n";
   ETL::showTime(); print "**************************************************************\n";
   ETL::showTime(); print "* WorkFlow Receiver Program ${VERSION}, DJ 2013 Copyright. *\n";
   ETL::showTime(); print "**************************************************************\n";
   print "\n";
